...
#transform according to the specified parameters of VGG19
...
def my_model(structure = 'vgg16', lr = 0.002):
...
model, criterion, optimizer = my_model()
print(model)
...
def predict(image_path, model, topk=5):
    ...
    probs = torch.topk(prob,topk)[0].tolist()[0]
    index = torch.topk(prob,topk)[1].tolist()[0]
    
    #index_to_class = {key: val for key, val in class_to_idx.items()}
    
    indx = []
    for i in range(len(model.class_to_idx.items())):
        indx.append(list(model.class_to_idx.items())[i][0])
        
    classes = []
    for i in range(5):
        classes.append(indx[index[i]])
